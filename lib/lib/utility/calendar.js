...
exports.simpleCollision = simpleCollision;
exports.collisionOrOrderMismatch = collisionOrOrderMismatch;
...
function simpleCollision(a, b, lineHeight) {
...
function collisionOrOrderMismatch(a, b, lineHeight) {
  var collisionPadding = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : EPSILON;
  // 2d collisions detection - https://developer.mozilla.org/en-US/docs/Games/Techniques/2D_collision_detection
  var verticalMargin = 0;
  return a.collisionLeft + collisionPadding < b.collisionLeft + b.collisionWidth && a.collisionLeft + a.collisionWidth - collisionPadding > b.collisionLeft && (a.top - verticalMargin + collisionPadding < b.top + b.height && a.top + a.height + verticalMargin - collisionPadding > b.top || a.top < b.top);
}
/**
...
function groupStack(lineHeight, item, group, groupHeight, groupTop, itemIndex, collision) {
...
function stackAll(itemsDimensions, groupOrders, lineHeight, stackItems) {
  ...
  for (var index in groupedItems) {
    ...
    var shouldStackEnforceOrder = group.shouldStackEnforceOrder !== undefined ? group.shouldStackEnforceOrder : false;

    var _stackGroup = stackGroup(_itemsDimensions, isGroupStacked, shouldStackEnforceOrder, lineHeight, groupTop),
...
function stackGroup(itemsDimensions, isGroupStacked, shouldStackEnforceOrder, lineHeight, groupTop) {
  var groupHeight = 0;
  var verticalMargin = 0;
  var collision = shouldStackEnforceOrder ? collisionOrOrderMismatch : simpleCollision; // Find positions for each item in group

  for (var itemIndex = 0; itemIndex < itemsDimensions.length; itemIndex++) {
    var r = {};

    if (isGroupStacked) {
      r = groupStack(lineHeight, itemsDimensions[itemIndex], itemsDimensions, groupHeight, groupTop, itemIndex, collision);
    } else {
